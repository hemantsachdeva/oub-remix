...
async def set_afk(afk_e):
    ...
    if string:
        AFKREASON = string
        await afk_e.edit(f"**SORRYY......! GTG!**\
        \nReason: `{string}`")
    else:
        await afk_e.edit("**SORRYYYY...! GTG!**")
    if user.last_name:
...
async def type_afk_is_not_true(notafk):
    ...
    if ISAFK:
        ISAFK = False
        msg = await notafk.respond("**HeManT is no longer AFK.**")
        time.sleep(3)
        await msg.delete()
    ...
    if mention.message.mentioned and not (await mention.get_sender()).bot:
        if ISAFK:
            ...
            if mention.sender_id not in USERS:
                if AFKREASON:
                    await mention.reply(f"HeManT is not active since {afk_since}.\
                        \nReason: `{AFKREASON}` \nPLeaSe WAiT")
                else:
                ...
                if USERS[mention.sender_id] % randint(2, 4) == 0:
                    if AFKREASON:
                        await mention.reply(f"HeManT is STiLL NoT AcTive since {afk_since}.\
                            \nReason: `{AFKREASON}` \nPLeaSe WAiT")
                    else:
                        await mention.reply(str(choice(AFKSTR)))
